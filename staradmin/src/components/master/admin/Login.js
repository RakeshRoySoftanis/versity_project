...
import { postAdminLoggedIn } from '../../../services/MasterServiceAdmin';
...
export class Login extends Component {
  ...
  handler = (e) => {
    ...
    // sent to server
    postAdminLoggedIn(data).then(response => {
      localStorage.setItem('msatoken', response.token); //token store on local storage
      this.setState({
        msloggedIn: true,
      });
  ...
  render() {
    ...
    if (this.state.msloggedIn) {
      return <Redirect push to="/admin/dashboard"></Redirect>
    }
